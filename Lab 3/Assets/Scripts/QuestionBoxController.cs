...
public class QuestionBoxController : MonoBehaviour
{
    ...
    public  GameObject mushroom; // the spawned mushroom prefab
    ...
    void  OnCollisionEnter2D(Collision2D col)
    {
        ...
        {
            hit = true;
            Debug.Log("Question Box is hit!!!");
            // spawn the mushroom prefab slightly above the box

            // this script attached to Box
            // "this.transform.position" will return world's position
            Instantiate(mushroom, new  Vector3(this.transform.position.x, this.transform.position.y  +  1.0f, this.transform.position.z), Quaternion.identity);
